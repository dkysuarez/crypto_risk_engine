...
class MertonJumpDiffusion:
    ...
    def simulate_correlated(
            ...
            correlation: float,
            jump_correlation: float = 0.8,  # NUEVO: correlación entre saltos
            T: float = 1.0,
            n_days: int = 252,
            n_simulations: int = 1000
        ...
        VERSIÓN DEFINITIVA: Brownianos + Saltos correlacionados
        """
        dt = T / n_days
        n_steps = n_days
        ...
        # ============= CORRELACIÓN BROWNIANA =============
        corr_matrix = np.array([[1.0, correlation], [correlation, 1.0]])
        L = np.linalg.cholesky(corr_matrix)

        # ============= CORRELACIÓN DE SALTOS =============
        jump_corr_matrix = np.array([[1.0, jump_correlation], [jump_correlation, 1.0]])
        L_jump = np.linalg.cholesky(jump_corr_matrix)
        ...
        for t in range(1, n_steps + 1):
            # 1. BROWNIANOS CORRELACIONADOS
            Z = np.random.randn(2, n_simulations)
            correlated_Z = L @ Z

            # 2. SALTOS CORRELACIONADOS (AMBAS DIMENSIONES)
            # 2.1 Intensidad de saltos (misma para ambos - correlación perfecta)
            lambda_avg = (self.lambda_jump + other_process.lambda_jump) / 2
            n_jumps = np.random.poisson(lambda_avg * dt, n_simulations)

            # 2.2 Tamaños de salto CORRELACIONADOS
            Z_jump = np.random.randn(2, n_simulations)
            correlated_Z_jump = L_jump @ Z_jump

            jump_sizes1 = self.mu_jump + self.sigma_jump * correlated_Z_jump[0, :]
            jump_sizes2 = other_process.mu_jump + other_process.sigma_jump * correlated_Z_jump[1, :]

            jump_component1 = jump_sizes1 * n_jumps
            jump_component2 = jump_sizes2 * n_jumps

            # 3. RETORNOS TOTALES
            returns1 = drift1 + diffusion_scale1 * correlated_Z[0, :] + jump_component1
            returns2 = drift2 + diffusion_scale2 * correlated_Z[1, :] + jump_component2
